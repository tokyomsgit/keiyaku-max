...
let caseQuery='';
// Search text: full/half width, case, spaces and small kana ignored, so "ｻﾝﾌﾟﾙ ﾏﾝｼｮﾝ 403" finds the
// registry's "サンプルマンシヨン架空 403号" (registries print ョ/ッ as full-size ヨ/ツ).
const SMALL_KANA={'ァ':'ア','ィ':'イ','ゥ':'ウ','ェ':'エ','ォ':'オ','ッ':'ツ','ャ':'ヤ','ュ':'ユ','ョ':'ヨ','ヮ':'ワ','ぁ':'あ','ぃ':'い','ぅ':'う','ぇ':'え','ぉ':'お','っ':'つ','ゃ':'や','ゅ':'ゆ','ょ':'よ','ゎ':'わ'};
const searchable=v=>String(v??'').normalize('NFKC').toLowerCase().replace(/\s+/g,'').replace(/[ァィゥェォッャュョヮぁぃぅぇぉっゃゅょゎ]/g,c=>SMALL_KANA[c]);
const caseText=c=>searchable([c.building_name,c.unit_name,c.address,c.owner,...(c.fields||[]).map(f=>f.value)].join(' '));
const shortDate=v=>{const d=new Date(v);return v&&!isNaN(d)?`${d.getFullYear()}/${d.getMonth()+1}/${d.getDate()}`:'';};
function caseRows(cases){
  const words=caseQuery.split(/[\s　]+/).map(searchable).filter(Boolean);
  const shown=cases.filter(c=>{const text=caseText(c);return words.every(w=>text.includes(w));});
  document.querySelector('#case-rows').innerHTML=shown.map(c=>`<tr class="case-row"><td><strong>${esc(c.building_name)}</strong></td><td>${esc(c.unit_name)}</td><td>${esc(c.address)}</td><td>${esc(shortDate(c.updated_at))}</td><td>${c.unresolved?`<span class="pill warn">${c.unresolved}件</span>`:'<span class="pill">確認済み</span>'}</td><td><button class="primary" data-open="${esc(c.id)}">内容を確認して生成</button></td></tr>`).join('')||'<tr><td colspan="6" class="muted">該当する物件がありません。検索する言葉を変えてください。</td></tr>';
  document.querySelector('#case-count').textContent=words.length?`${cases.length}件中 ${shown.length}件`:`${cases.length}件`;
  root.querySelectorAll('[data-open]').forEach(button=>button.onclick=()=>detail(button.dataset.open));
}
function list(){
  const cases=[...(liveState?.cases||[])].sort((a,b)=>String(b.updated_at||'').localeCompare(String(a.updated_at||'')));
  root.innerHTML=`<nav class="steps" aria-label="契約書作成の工程"><a>① 資料</a><a>② 要確認</a><a class="active" aria-current="step">③ 契約書生成</a></nav><div class="top"><div><h1>契約書を生成</h1><p>登録済みの物件から契約書を作れます。初めての物件は「新しい契約書を作成」から謄本を入れてください。</p></div><div class="actions"><button id="create" class="primary">新しい契約書を作成</button><button id="refresh">最新情報に更新</button></div></div>${cases.length?`<div class="card"><div class="case-search"><label for="case-search">登録済みの物件を探す</label><input id="case-search" type="search" autocomplete="off" placeholder="物件名・号室・所在地・家屋番号・所有者で検索（例：架空 403）" value="${esc(caseQuery)}"><span id="case-count" class="muted"></span></div><div class="table-scroll"><table><thead><tr><th>物件名</th><th>号室</th><th>所在地</th><th>更新日</th><th>要確認</th><th>操作</th></tr></thead><tbody id="case-rows"></tbody></table></div></div>`:'<div class="card empty"><p>生成できる物件データがありません。</p><p>「新しい契約書を作成」から登録してください。</p></div>'}`;
  document.querySelector('#refresh').onclick=load;
  document.querySelector('#create').onclick=()=>createForm();
  const search=document.querySelector('#case-search');
  if(search){search.oninput=()=>{caseQuery=search.value;caseRows(cases);};caseRows(cases);}
}

let targetCase=null;
function createForm(caseId=null){forgetJob();targetCase=caseId;newPdfs=[];root.innerHTML=`<nav class="steps"><a class="active">① 資料</a><a>② 要確認</a><a>③ 契約書生成</a></nav><p><button id="back">← 案件一覧へ</button></p><div class="card"><h1>${caseId?'資料を追加':'新しい契約書を作成'}</h1><p>登記簿謄本（建物・土地）のPDFを追加してください。一度にまとめても、1件ずつ続けて追加しても構いません。謄本以外の資料（重調・管理規約・重要事項説明書・用途地域など）は読み込みません。</p><label class="pdf-drop" id="pdf-drop"><strong>謄本PDFをここにドラッグ</strong><span>またはクリックして選択（複数可）</span><input id="pdf-files" type="file" accept="application/pdf,.pdf" multiple hidden></label><div id="file-list"></div><div class="actions"><button id="start-new" class="primary" disabled>選択した資料を読み取る</button></div><p id="status" role="status"></p><details id="manual"><summary>PDFから判定できない場合のみ手入力</summary><div class="facts new-case-form"><label>物件名<input id="building-name"></label><label>号室<input id="unit-name"></label><label>登記上の所在<input id="registry-location"></label><label>家屋番号<input id="house-number"></label><label>住居表示（任意）<input id="display-address"></label></div><div class="actions"><button id="save-new">手入力で作成</button></div></details></div>`;document.querySelector('#back').onclick=list;const input=document.querySelector('#pdf-files'),drop=document.querySelector('#pdf-drop');input.onchange=()=>{selectPdfs(input.files);input.value='';};drop.ondragover=e=>{e.preventDefault();drop.classList.add('is-dragging')};drop.ondragleave=()=>drop.classList.remove('is-dragging');drop.ondrop=e=>{e.preventDefault();drop.classList.remove('is-dragging');selectPdfs(e.dataTransfer.files)};document.querySelector('#file-list').onclick=e=>{const button=e.target.closest('[data-remove-pdf]');if(button)removePdf(Number(button.dataset.removePdf));};document.querySelector('#start-new').onclick=importPdfs;document.querySelector('#save-new').onclick=createNew;}
...
// For now only 謄本 is read (see ACTIVE_KINDS in cloud_worker.py). Add kinds back here and set READ_ZONING to re-enable.
const KIND_LABELS={registry:'登記簿謄本（建物・土地）',skip:'この資料は読み取らない'};
...
async function generate(item){
  ...
  try{
    ...
    link.href=url;link.download=`契約書_${item.building_name}_${item.unit_name}.xlsx`;link.click();setTimeout(()=>URL.revokeObjectURL(url),60000);
