...
function createForm(){root.innerHTML=`<nav class="steps"><a class="active">① 資料</a><a>② 要確認</a><a>③ 契約書生成</a></nav><p><button id="back">← 案件一覧へ</button></p><div class="card"><h1>新しい契約書を作成</h1><p>同じ物件のPDFをまとめて入れてください。</p><label class="pdf-drop" id="pdf-drop"><strong>PDFをここにドラッグ</strong><span>またはクリックして選択（複数可）</span><input id="pdf-files" type="file" accept="application/pdf,.pdf" multiple hidden></label><div id="file-list"></div><div class="actions"><button id="start-new" class="primary" disabled>資料を読み取って始める</button></div><p id="status" role="status"></p><details id="manual"><summary>PDFから判定できない場合のみ手入力</summary><div class="facts new-case-form"><label>物件名<input id="building-name"></label><label>号室<input id="unit-name"></label><label>登記上の所在<input id="registry-location"></label><label>家屋番号<input id="house-number"></label><label>住居表示（任意）<input id="display-address"></label></div><div class="actions"><button id="save-new">手入力で作成</button></div></details></div>`;document.querySelector('#back').onclick=list;const input=document.querySelector('#pdf-files'),drop=document.querySelector('#pdf-drop');input.onchange=()=>selectPdfs(input.files);drop.ondragover=e=>{e.preventDefault();drop.classList.add('is-dragging')};drop.ondragleave=()=>drop.classList.remove('is-dragging');drop.ondrop=e=>{e.preventDefault();drop.classList.remove('is-dragging');selectPdfs(e.dataTransfer.files)};document.querySelector('#start-new').onclick=importPdfs;document.querySelector('#save-new').onclick=createNew;}

let newPdfs=[];
function selectPdfs(files){newPdfs=Array.from(files).filter(file=>file.type==='application/pdf'||file.name.toLowerCase().endsWith('.pdf')).slice(0,10);document.querySelector('#file-list').innerHTML=newPdfs.map(file=>`<p>✓ ${esc(file.name)} <span class="muted">${(file.size/1024/1024).toFixed(1)}MB</span></p>`).join('');document.querySelector('#start-new').disabled=!newPdfs.length;}
