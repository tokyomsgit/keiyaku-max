...
async function api(path,body){const r=await fetch(path,{method:body?'POST':'GET',headers:body?{'Content-Type':'application/json','X-CSRF-Token':csrf}:{},body:body?JSON.stringify(body):undefined});const d=await r.json();if(!r.ok)throw Error(d.error||'処理に失敗しました。');return d;}
function render(){
 if(!state)return;
 document.querySelector('#mode').innerHTML=pill(state.mode==='demo'?'DEMO':'Supabase接続');
 document.querySelector('#notice').textContent=state.mode==='demo'?(state.db_write_enabled?'デモ表示・DB更新有効：採用操作は実データを更新します。':'デモモード：解析済みJSONを使用しています。採用・保留はこの起動中だけの操作で、DBは更新しません。'):'';
