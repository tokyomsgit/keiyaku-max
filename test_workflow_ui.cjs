...
ui.renderDetail(c,panel);assert(panel.innerHTML.includes(' multiple hidden disabled'));assert(panel.innerHTML.includes('購入時重説／謄本／重調／管理規約'));
const reviewCase={...c,diffs:[{id:'d1',code:'management_fee',label:'管理費',old_value:12000,new_value:13500,old_date:'2025-01-01',new_date:'2026-01-01',recommended_source:'new',review_status:'unreviewed',can_adopt:true,source:'最新重調'},{id:'d2',code:'repair_reserve_fee',label:'修繕積立金',old_value:8000,new_value:9000,review_status:'unreviewed',can_adopt:true,source:'最新重調'}]};
ui.renderReview(reviewCase,panel);assert(panel.innerHTML.includes('1 / 2件 選択済み'));assert(panel.innerHTML.includes('管理費'));assert(panel.innerHTML.includes('修繕積立金'));assert(panel.innerHTML.includes('（推奨）'));assert(panel.innerHTML.includes('value="new" checked'));assert(/data-action="confirm-reviews" disabled/.test(panel.innerHTML));assert(!panel.innerHTML.includes('契約書生成へ'));
